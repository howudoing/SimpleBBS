...
class Articles(models.Model):
    ...
    status_choices = (('draft', u'草稿'),
                      ('published', u'已发布'),
                      ('hidden', u'隐藏'))
    status = models.CharField(choices=status_choices, default='published', max_length=16)

    def __str__(self):
        return self.title
    def clean(self):
        if self.status == 'draft' and self.pub_date is not None:
            raise ValidationError('Draft entries may not have a publish datetime')
        if self.status == 'published' and self.pub_date is None:
            self.pub_date = datetime.date.today()


class Comment(models.Model):
    articles = models.ForeignKey(Articles, verbose_name=u"所属文章",on_delete=models.CASCADE)
    ...
    def __str__(self):
        return "%s,P:%s,%s" % (self.articles, self.parent_comment, self.comment)
